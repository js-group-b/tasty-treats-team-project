...
import axios from 'axios';
import { searchParams } from './cards.js'; // searchParams başka bir dosyadan alınıyor
import { fetchAndMapCardsData } from './cards.js'; // Verileri işlemek için kullanılan fonksiyon

const allIngredientsURL =
  'https://tasty-treats-backend.p.goit.global/api/ingredients';

// Tüm malzemeleri getiren fonksiyon
async function fetchIngredients() {
  try {
    const urlIngredient = `${allIngredientsURL}`;
    const response = await axios.get(urlIngredient);
    const ingredients = response.data;

    // Ingredient seçeneklerini bulma
    const ingredientList = document.querySelector('#ingredient-options');
    const ingredientTrigger = document.querySelector('#ingredient-trigger');

    // Başlangıçta listeyi gizle
    ingredientList.style.display = 'none';

    // Her malzeme için bir liste öğesi oluştur
    ingredients.forEach(ingredient => {
      const inglistItem = document.createElement('li');
      inglistItem.textContent = ingredient.name;
      inglistItem.setAttribute('data-id', ingredient._id);
      inglistItem.setAttribute('tabindex', '0');
      ingredientList.appendChild(inglistItem);

      // Her malzeme öğesine tıklama olayı ekle
      inglistItem.addEventListener('click', async (event) => {
        // Seçilen malzeme adı
        const selectedIngredient = event.target.textContent;

        // Search params'ı güncelle
        searchParams.set('ingredient', selectedIngredient);

        // Veriyi çek
         fetchAndMapCardsData(); // Veriyi çekmek için daha önce tanımlanan fonksiyon

        // Listeyi gizle
        ingredientList.style.display = 'none';

        // Butondaki metni seçilen malzeme ile değiştir
        document.querySelector('#ingredient-trigger-text').textContent =
          selectedIngredient;
      });
    });

    // Butonla listeyi açıp kapama işlevi
    ingredientTrigger.addEventListener('click', () => {
