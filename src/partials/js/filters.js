...
// Filtrelerin güncellenmesi için fonksiyon
const updateSearchParams = () => {
  const filterData = getSelectedFilterData();

  searchParams.set('title', filterData.searchValue);
  searchParams.set('time', filterData.timeValue);
  searchParams.set('area', filterData.areaValue);
  searchParams.set('ingredient', filterData.ingredientValue);

  searchParams.set('page', 1); // Sayfayı resetle
  fetchAndMapCardsData(); // Yeni veriyi çek
};

// Seçilen filtreleri almak için fonksiyon
const getSelectedFilterData = () => {
  const searchValue = document.getElementById('search').value;
  const selectedTime = document.querySelector('.time-container .selected');
  const timeValue = selectedTime ? selectedTime.textContent.trim() : '';
  const selectedArea = document.querySelector('.area-container .selected');
  const areaValue = selectedArea ? selectedArea.textContent.trim() : '';
  const selectedIngredient = document.querySelector(
    '.ingr-container .selected'
  );
  const ingredientValue = selectedIngredient
    ? selectedIngredient.textContent.trim()
    : '';
...
const updateSelectedOption = (optionSelector, triggerTextId) => {
  ...
  options.forEach(option => {
    option.addEventListener('click', () => {
      const selectedText = option.textContent;
      const triggerText = document.getElementById(triggerTextId);
      const triggerElement = option
        .closest('.filter-custom-select-container')
        .querySelector('.filter-custom-select-trigger');
      ...
      option
        .closest('.filter-custom-select-container')
        .querySelector('ul')
        .classList.add('filter-hidden-dropdown');
...
if (resetButton) {
  resetButton.addEventListener('click', () => {
    ...
    // 4. Resetleme sonrası verileri tekrar çek
